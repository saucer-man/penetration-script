...
def rsync(host, ports=[873]):
    ...
    for port in ports:
        ...
        except ConnectionRefusedError: # 端口未开， 由于目标计算机积极拒绝，无法连接
            pass
        except TimeoutError:  # 主机未开， 由于连接方在一段时间后没有正确答复或连接的主机没有反应，连接尝试失败
            pass
        except socket.timeout:
            pass
        except:
...
#             r = requests.get(upload_url, timeout=5, allow_redirects=False, verify=False)
...
#                 # print(data)
#                 r = requests.post(url=url_vuln, headers=headers, data=data, timeout=5, verify=False)
#                 if r.status_code == 200 and "Installed" in r.text:
#                     result.append(f"atlassian crowd: {host}:{port}-->{r.content}")
#         except requests.exceptions.ConnectionError:  # host没开/端口没开
#             pass
#         except:
#             traceback.print_exc()
#     return result
...
def elasticsearch(host, ports = [9200]):
    ...
    for port in ports:
        try:
            r = requests.get(f"http://{host}:{port}", timeout=5, allow_redirects=False, verify=False)
...
def hadoop(host, ports=[8088]):
    ...
    for port in ports:
        try:
            r = requests.get(f"http://{host}:{port}/cluster", timeout=5, allow_redirects=False, verify=False)
            if "Hadoop" in r.text:
...
def docker(host, ports=[2375]):
    ...
    for port in ports:
        try:
            r = requests.get(f"http://{host}:{port}/version", timeout=5, verify=False)
